function [OUT] = Bounds_Computation_Verification(Orig, Target, Known_Bounds, Domain)
...
if H == 0
 ...
 prog = sosineq(prog, Bound_Up -betasym); % Upper bound on the objective function
 prog = sosineq(prog, -Bound_Low + betasym); % Lower bound on the objective function
 prog = sosineq(prog, B0 - expand(sig_u*polyq2_out) - 1); %B1 is greater than 1 outside of the underapproximation of q2
 stdvar = gx;
